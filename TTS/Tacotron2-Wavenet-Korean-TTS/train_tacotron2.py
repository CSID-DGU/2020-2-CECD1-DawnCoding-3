...
def add_stats(model, model2=None, scope_name='train'):
    with tf.variable_scope(scope_name) as scope:
        ...
        if scope_name == 'train':
            gradient_norms = [tf.norm(grad)
                              for grad in model.gradients if grad is not None]
...
def save_and_plot_fn(args, log_dir, step, loss, prefix):
    ...
    if 'korean_cleaners' in [x.strip() for x in hparams.cleaners.split(',')]:
        ...
        plot.plot_alignment(align, align_path, info=info_text, text=sequence_to_text(
            seq, skip_eos_and_pad=True, combine_jamo=True), isKorean=True)
    else:
        log('Training non-korean : X use jamo')
        plot.plot_alignment(align, align_path, info=info_text, text=sequence_to_text(
            seq, skip_eos_and_pad=True, combine_jamo=False), isKorean=False)


def save_and_plot(sequences, spectrograms, alignments, log_dir, step, loss, prefix):

    fn = partial(save_and_plot_fn, log_dir=log_dir,
                 step=step, loss=loss, prefix=prefix)
...
def train(log_dir, config):
    ...
    if num_speakers > 1 and hparams.model_type not in ["multi-speaker", "simple"]:
        raise Exception(
            "[!] Unkown model_type for multi-speaker: {}".format(config.model_type))
    ...
    # 'logdir-tacotron\\moon_2018-08-28_13-06-42'
    ...
    with tf.variable_scope('datafeeder') as scope:
        ...
        train_feeder = DataFeederTacotron2(
            coord, data_dirs, hparams, config, 32, data_type='train', batch_size=config.batch_size)
        test_feeder = DataFeederTacotron2(
            coord, data_dirs, hparams, config, 8, data_type='test', batch_size=config.num_test)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % config.test_interval == 0:
                    ...
                    save_and_plot(
                        sequences[:1], spectrograms[:1], alignments[:1], log_dir, step, loss, "train")
...
def main():
    ...
    # parser.add_argument(
    #     '--data_paths', default='C:\\Users\\jw969\\Desktop\\project\\Tacotron2-Wavenet-Korean-TTS\\data\\moon,C:\\Users\\jw969\\Desktop\\project\\Tacotron2-Wavenet-Korean-TTS\\data\\son')
    
    parser.add_argument(
        '--data_paths', default='/home/aimaster/Tacotron2-Wavenet-Korean-TTS/data/moon,/home/aimaster/Tacotron2-Wavenet-Korean-TTS/data/son')
    ...
    # parser.add_argument('--load_path', default='logdir-tacotron2\\moon+son')

    # ckpt로 부터 model을 restore하지만, global step은 0에서 시작
    ...
    parser.add_argument('--skip_path_filter', type=str2bool,
                        default=False, help='Use only for debugging')
